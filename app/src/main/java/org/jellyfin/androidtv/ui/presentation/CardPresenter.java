...
public class CardPresenter extends Presenter {
    ...
    private final Lazy<RatingsRepository> ratingsRepository = KoinJavaComponent.<RatingsRepository>inject(RatingsRepository.class);
    ...
    public void onBindViewHolder(Presenter.ViewHolder viewHolder, Object item) {
        ...
        if (rowItem instanceof AudioQueueBaseRowItem && ((AudioQueueBaseRowItem) rowItem).getPlaying()) {
            ...
            if (rowItem.getBaseItem() != null && rowItem.getBaseItem().getType() != BaseItemKind.USER_VIEW) {
                ...
                } else if (ratingType == RatingType.RATING_PLUGIN) {
                    // Show community ratings from the Ratings Plugin using overlay badge
                    BaseItemKind itemType = rowItem.getBaseItem().getType();
                    if (itemType == BaseItemKind.MOVIE || itemType == BaseItemKind.SERIES || itemType == BaseItemKind.EPISODE) {
                        java.util.UUID itemId = rowItem.getBaseItem().getId();
                        RatingStats stats = ratingsRepository.getValue().getCachedStats(itemId);
                        if (stats != null && stats.getTotalRatings() > 0) {
                            // Use the overlay badge that's visible on Netflix cards
                            holder.mCardView.setRatingBadge(String.format(Locale.US, "%.1f", stats.getAverageRating()));
                        } else {
                            // Fetch async and update when ready
                            ratingsRepository.getValue().fetchStatsAsync(itemId, fetchedStats -> {
                                // Check if the card is still showing the same item
                                BaseRowItem currentItem = holder.getItem();
                                if (currentItem != null && currentItem.getBaseItem() != null &&
                                    currentItem.getBaseItem().getId().equals(itemId) &&
                                    fetchedStats != null && fetchedStats.getTotalRatings() > 0) {
                                    // Use the overlay badge that's visible on Netflix cards
                                    holder.mCardView.setRatingBadge(String.format(Locale.US, "%.1f", fetchedStats.getAverageRating()));
                                }
