...
// Rating functions for custom ratings plugin integration

fun FullDetailsFragment.loadRatingStats(
	itemId: UUID,
	forceRefresh: Boolean = false,
	callback: (stats: RatingStats?) -> Unit,
) {
	val ratingsRepository by inject<RatingsRepository>()

	lifecycleScope.launch {
		val stats = ratingsRepository.getRatingStats(itemId, forceRefresh)
		callback(stats)
	}
}

fun FullDetailsFragment.submitRating(
	itemId: UUID,
	rating: Int,
	...
	lifecycleScope.launch {
		val success = ratingsRepository.submitRating(itemId, rating)
		callback(success)
	}
}

fun FullDetailsFragment.deleteRating(
	itemId: UUID,
	callback: (success: Boolean) -> Unit,
) {
	val ratingsRepository by inject<RatingsRepository>()
	...
	lifecycleScope.launch {
		val success = ratingsRepository.deleteRating(itemId)
		callback(success)
	}
}

fun FullDetailsFragment.loadUserReviews(
	itemId: UUID,
	callback: (reviews: List<org.jellyfin.androidtv.data.ratings.UserRatingDetail>?) -> Unit,
) {
	val ratingsRepository by inject<RatingsRepository>()

	Timber.i("REVIEWS: loadUserReviews fetching for itemId=$itemId")
	lifecycleScope.launch {
		val reviews = ratingsRepository.getDetailedRatings(itemId)
		Timber.i("REVIEWS: got ${reviews?.size ?: 0} total reviews from API")
		// Filter to only reviews with text and sort by (likes - dislikes)
		val sortedReviews = reviews
			?.filter { it.hasReview && !it.reviewText.isNullOrBlank() }
			?.sortedByDescending { it.likeCount - it.dislikeCount }
		Timber.i("REVIEWS: after filtering, ${sortedReviews?.size ?: 0} reviews with text")
		callback(sortedReviews)
	}
}

fun FullDetailsFragment.likeReview(
	reviewerUserId: String,
	itemId: UUID,
	isLike: Boolean,
	callback: (response: org.jellyfin.androidtv.data.ratings.LikeResponse?) -> Unit,
) {
	val ratingsRepository by inject<RatingsRepository>()

	lifecycleScope.launch {
		val response = ratingsRepository.likeReview(reviewerUserId, itemId, isLike)
		callback(response)
	}
}

fun FullDetailsFragment.loadReviewComments(
	reviewerUserId: String,
	itemId: UUID,
	callback: (comments: List<org.jellyfin.androidtv.data.ratings.ReviewComment>?) -> Unit,
) {
	val ratingsRepository by inject<RatingsRepository>()

	lifecycleScope.launch {
		val comments = ratingsRepository.getReviewComments(reviewerUserId, itemId)
		callback(comments)
	}
}

fun FullDetailsFragment.submitComment(
	reviewerUserId: String,
	itemId: UUID,
	commentText: String,
	callback: (success: Boolean) -> Unit,
	...
	lifecycleScope.launch {
		val success = ratingsRepository.addComment(reviewerUserId, itemId, commentText)
		callback(success)
	}
}
