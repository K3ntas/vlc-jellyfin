...
import org.jellyfin.playback.vlc.VlcBackendSelector
import org.jellyfin.playback.vlc.VlcPlayerOptions
import org.jellyfin.playback.vlc.vlcPlayerPlugin
...
fun Scope.createPlaybackManager() = playbackManager(androidContext()) {
	...
	// VLC backend (for ASS/SSA subtitles)
	val vlcPlayerOptions = VlcPlayerOptions(
		enableDebugLogging = userPreferences[UserPreferences.debuggingEnabled],
		fontDirectory = androidContext().cacheDir.resolve("fonts").absolutePath,
	)
	install(vlcPlayerPlugin(get(), vlcPlayerOptions))

	// Backend selector: use VLC for ASS/SSA subtitles if enabled
	val useVlcForAss = userPreferences[UserPreferences.useVlcForAssSubtitles]
	backendSelector = VlcBackendSelector(enabled = useVlcForAss)
