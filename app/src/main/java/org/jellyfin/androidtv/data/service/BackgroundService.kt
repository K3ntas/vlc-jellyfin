...
class BackgroundService(
	...
	companion object {
		...
		/**
		 * Moving along a row fires a selection for every card passed over. Without a pause before
		 * loading, each one starts decoding backdrops that are discarded a moment later.
		 */
		val LOAD_DEBOUNCE = 350.milliseconds

		/**
		 * Backdrops are blurred behind the UI, so asking the server for a full resolution image
		 * only costs bandwidth and decode time that the blur throws away.
		 */
		const val BACKDROP_DOWNSCALE = 2
	}
	...
	fun setBackground(baseItem: BaseItemDto?) {
		...
		// Get all backdrop urls, letting the server do the downscaling
		val metrics = context.resources.displayMetrics
		val backdropUrls = (baseItem.itemBackdropImages + baseItem.parentBackdropImages)
			.map {
				it.getUrl(
					api,
					fillWidth = metrics.widthPixels / BACKDROP_DOWNSCALE,
					fillHeight = metrics.heightPixels / BACKDROP_DOWNSCALE,
				)
			}
			.toSet()
	...
	private fun loadBackgrounds(backdropUrls: Set<String>) {
		...
		loadBackgroundsJob = scope.launch(Dispatchers.IO) {
			// Cancelled by the next selection, so nothing loads until the user stops moving
			delay(LOAD_DEBOUNCE)

			// Show the first backdrop before decoding the rest. Only one is ever on screen until
			// the slideshow advances, so decoding the others up front just delays the first paint.
			val first = loadBackdrop(backdropUrls.first())
			_backgrounds = listOfNotNull(first)
			_currentIndex = 0
			update()

			if (backdropUrls.size > 1) {
				val remaining = backdropUrls.drop(1).mapNotNull { loadBackdrop(it) }
				_backgrounds = listOfNotNull(first) + remaining

				// Re-run so the slideshow timer starts now that there is more than one
				update()
			}
		}
	}
