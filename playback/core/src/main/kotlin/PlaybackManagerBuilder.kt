...
import org.jellyfin.playback.core.backend.DefaultBackendSelector
...
class PlaybackManagerBuilder(context: Context) {
	...
	var backendSelector: BackendSelector = DefaultBackendSelector()
	...
	fun build(): PlaybackManager {
		...
		// Support multiple backends with selection logic
		require(backends.isNotEmpty()) { "At least one backend must be provided" }
		...
		return PlaybackManager(backends, backendSelector, services, options)
	}
}
