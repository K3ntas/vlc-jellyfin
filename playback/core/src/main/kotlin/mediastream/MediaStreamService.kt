...
internal class MediaStreamService(
	...
	override suspend fun onInitialize() {
		manager.queue.entry.onEach { entry ->
			...
			if (entry == null) {
				manager.backendService.backend?.stop()
			} else {
				val hasMediaStream = entry.ensureMediaStream()

				if (hasMediaStream) {
					// Select the best backend for this stream
					val stream = requireNotNull(entry.mediaStream)
					manager.backendService.switchToBackendForStream(stream)

					// Play on the selected backend
					val backend = requireNotNull(manager.backendService.backend)
